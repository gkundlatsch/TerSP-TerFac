...
estimators_list    = [4000]
lr_list            = [0.001]
depth_list         = [5]
child_weight_list  = [3]        # for min_child_weight
subsample_list     = [0.65]  # for subsample

param_grid = {
    'n_estimators':     estimators_list,
    'learning_rate':    lr_list,
    'max_depth':        depth_list,
    'min_child_weight': child_weight_list,
...
# ────────────────────────────────────────────────────────────────────────────────
# 3) RUN GRIDSEARCHCV (with train+test R²)
# ────────────────────────────────────────────────────────────────────────────────
...
grid = GridSearchCV(
    ...
    scoring='r2',
    cv=cv,
    return_train_score=True,  # include train R² in cv_results_
    n_jobs=-1
)
grid.fit(X, y)

# ────────────────────────────────────────────────────────────────────────────────
# 4) EXPORT ALL PARAMS + TRAIN/TEST R² TO EXCEL
# ────────────────────────────────────────────────────────────────────────────────
...
export_df.to_excel('grid_search_all5params_with_train_test_R2_best_f.xlsx', index=False)

# ────────────────────────────────────────────────────────────────────────────────
# 5) BUILD PIVOT AND PLOT BIG HEATMAP
# ────────────────────────────────────────────────────────────────────────────────
...
row_tuples = pivot.index.to_list()
col_tuples = pivot.columns.to_list()
...
ax.set_xlabel('N Estimators, Learning Rate', fontsize=12)
ax.set_ylabel('Max Depth, Min Child Weight, Subsample', fontsize=12)
ax.set_title('CV mean R² over 5 hyperparameters', fontsize=14)

cbar = fig.colorbar(im, ax=ax)
cbar.set_label('Mean test R²', rotation=270, labelpad=15)

plt.tight_layout()
fig.savefig('small_heatmap_all5params_Best.png', dpi=300)
plt.close(fig)
