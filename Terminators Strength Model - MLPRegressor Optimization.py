...
# ─── 3) Run 5-fold CV for each architecture ────────────────────────────────────
...
for arch in architectures:
    ...
    print(f"Running 5-fold CV for architecture {arch}…")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        # Predict on fold's train and test
        train_preds = mlp.predict(X_train_fold)
        test_preds  = mlp.predict(X_test_fold)
        
        # Compute metrics
        train_r2 = r2_score(y_train_fold, train_preds)
        train_mse = mean_squared_error(y_train_fold, train_preds)
        test_r2 = r2_score(y_test_fold, test_preds)
        test_mse = mean_squared_error(y_test_fold, test_preds)
        ...
        print(
            f"  Fold {fold_idx+1}: "
            f"train R²={train_r2:.4f}, train MSE={train_mse:.2f}, "
            f"test R²={test_r2:.4f}, test MSE={test_mse:.2f}"
        )
    
    # Average and standard deviation across folds
    mean_train_r2 = np.mean(fold_train_r2)
    std_train_r2  = np.std(fold_train_r2, ddof=1)
    mean_train_mse = np.mean(fold_train_mse)
    std_train_mse  = np.std(fold_train_mse, ddof=1)
    
    mean_test_r2 = np.mean(fold_test_r2)
    std_test_r2  = np.std(fold_test_r2, ddof=1)
    mean_test_mse = np.mean(fold_test_mse)
    std_test_mse  = np.std(fold_test_mse, ddof=1)
    
    cv_results.append({
        'architecture': "_".join(str(n) for n in arch),
        'train_r2_mean': mean_train_r2,
        'train_r2_std': std_train_r2,
        'train_mse_mean': mean_train_mse,
        'train_mse_std': std_train_mse,
        'test_r2_mean': mean_test_r2,
        'test_r2_std': std_test_r2,
        'test_mse_mean': mean_test_mse,
    ...
    print(
        ...
        f"train MSE = {mean_train_mse:.2f} ± {std_train_mse:.2f} | "
        f"test R² = {mean_test_r2:.4f} ± {std_test_r2:.4f}, "
        f"test MSE = {mean_test_mse:.2f} ± {std_test_mse:.2f}\n"
    )

# ─── 4) Build summary DataFrame and export to Excel ────────────────────────────
...
summary_df.to_excel("mlp_5fold_performance.xlsx", index=False)
print("Exported 5-fold CV summary to 'mlp_5fold_performance.xlsx'.")

# ─── 5) (Optional) Plot mean test R² ± 1 std ───────────────────────────────────

# 1) Sort by test_r2_mean if you want architectures ordered by Test R² (highest first)
summary_df = summary_df.sort_values(by='test_r2_mean', ascending=False).reset_index(drop=True)

labels = summary_df['architecture']
x = np.arange(len(labels))      # label locations on x-axis
width = 0.35                    # width of each bar

means_train = summary_df['train_r2_mean']
errs_train = summary_df['train_r2_std']
means_test = summary_df['test_r2_mean']
errs_test = summary_df['test_r2_std']
...
ax.set_title("Train vs. Test R\u00B2 Comparison by Architecture", fontsize=14)
ax.set_xticks(x)
ax.set_xticklabels(labels, rotation=45, ha='right')
ax.set_ylim(
    0.0,
    max(means_train.max() + errs_train.max(), means_test.max() + errs_test.max()) + 0.05
)
ax.legend()

ax.grid(axis='y', linestyle='--', alpha=0.5)
fig.tight_layout()
plt.savefig("mlp_train_vs_test_r2_grouped.png", dpi=300)
plt.show()
